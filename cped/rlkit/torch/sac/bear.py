...
class BEARTrainer(TorchTrainer):
    ...
    def train_from_torch(self, batch):
        ...
        """
        recon, mean, std = self.vae(s,x)
        recon_loss = self.qf_criterion(recon, s)
        ...
        if self._n_train_steps_total >= 40000:
            ...
            sampled_x, raw_sampled_x = self.vae.decode_multiple(s, num_decode=1000)
            density_x = st.multivariate_normal.pdf(sample_x, mean, cov)
            density_raw=st.multivariate_normal.pdf(x, mean, cov)
            out = np.concatenate((x,density_raw,sample_x,density_x), axis=1)
            np.savetxt('vae.csv', a, delimiter=",")
        ...
        # if self._n_train_steps_total >= 40000:
            ...
            # if self.mode == 'auto':
                # if self.policy_update_style == '0':
                   # policy_loss = (-torch.min(q_val1, q_val2)[:, 0] + self.log_alpha.exp().detach() * (mmd_loss - self.target_mmd_thresh)).mean()
                   # alpha_loss=-(-torch.min(q_val1.detach(), q_val2.detach())[:, 0] + self.log_alpha.exp() * (mmd_loss - self.target_mmd_thresh).detach()).mean()
                # elif self.policy_update_style == '1':
                     # policy_loss = (-torch.mean(q_val1, q_val2)[:, 0] + self.log_alpha.exp().detach() * (mmd_loss - self.target_mmd_thresh)).mean()
                     # alpha_loss=-(-torch.mean(q_val1.detach(), q_val2.detach())[:, 0] + self.log_alpha.exp() * (mmd_loss - self.target_mmd_thresh).detach()).mean()
            # else:
                # if self.policy_update_style == '0':
                   # policy_loss = (-torch.min(q_val1, q_val2)[:, 0] + 100*mmd_loss).mean()
                # elif self.policy_update_style == '1':
                     # policy_loss = (-torch.mean(q_val1, q_val2)[:, 0] + 100*mmd_loss).mean()
        # else:
            # if self.mode == 'auto':
                # policy_loss = (self.log_alpha.exp().detach() * (mmd_loss - self.target_mmd_thresh)).mean()
                # alpha_loss=-(self.log_alpha.exp() * (mmd_loss - self.target_mmd_thresh).detach()).mean()
            # else:
        ...
        # if self.mode == 'auto':
            ...
            # #print(self.policy.last_fc.bias.grad)
