...
class UWAC_DropoutTrainer(TorchTrainer):
    def __init__(
        self,
        env,
        policy,
    ...
    def train_from_torch(self, batch):
        ...
        with torch.no_grad():
            ...
            if self.var_Pi:
                weight = self._get_weight(q_var.detach(),self.beta).squeeze()
            else:
                weight = 1.
        ...
        if self._n_train_steps_total >= 40000:
            ...
            if self.mode == 'auto':
                policy_loss = (-policy_loss*weight.detach() + self.log_alpha.exp().detach() * (mmd_loss - self.target_mmd_thresh)).mean()
                alpha_loss=-((-policy_loss*weight).detach() + self.log_alpha.exp() * (mmd_loss - self.target_mmd_thresh).detach()).mean()
            else:
