...
class NICE(nn.Module):
    ...
    def __init__(self,mask_type,xdim,seed=0,init_type= "uniform",hidden_size=750, no_of_layers=4,hidden_layer=2):
        ...
        self.scale_factor = nn.Parameter(torch.zeros(1, xdim, dtype=torch.float32, requires_grad=True).to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')))
...
class RealNVP(nn.Module):
    ...
    def forward(self,  *inputs,train=False):
        ...
        for i in range(self.no_of_layers):
            split_value=int(int_shape(x)[1]/2)
            b = torch.cat([torch.ones(split_value), torch.zeros(self.xdim-split_value)], 0).to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
            x1 = x[:, :split_value]
            x2 = x[:, split_value:]
            if self.mask_type == 'checkerboard0':
